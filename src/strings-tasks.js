...
function removeFirstOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    ...
    const str1 = str.substring(0, ind);
    const str2 = str.substring(ind + value.length, str.length);
    newStr = str1 + str2;
    return newStr;
  }
  return str;
...
function reverseWords(str) {
  const arr = str.split(' ');
  const newStr = arr.map((word) => word.split('').reverse().join(''));
  return newStr.join(' ');
}
